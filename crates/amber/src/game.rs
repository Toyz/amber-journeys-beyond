...
///
/// Lived in the desktop front end until the browser one went without it and
/// every room's ambience stacked on the last: the loops are the room's, not
/// the window's, so this belongs with the game.
///
...
pub fn update_ambience(game: &mut Game, audio: Option<&crate::audio::Audio>) {
